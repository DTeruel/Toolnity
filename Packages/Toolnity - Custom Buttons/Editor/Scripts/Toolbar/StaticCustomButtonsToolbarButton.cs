...
namespace Toolnity.CustomButtons
{
	...
	public class StaticCustomButtonsToolbarButton : VisualElement
	{
		public const string ID = "Toolnity/StaticCustomButtons";
		private const string DEFAULT_ICON = "cs Script Icon";
		
		private static StaticCustomButtonsToolbarButton instance;

		private StaticCustomButtonsToolbarButton()
		{
		...
		private static void AddButtonsAfterScriptsCompile()
		{
			...
			if (instance == null)
			{
				instance = new StaticCustomButtonsToolbarButton();
			}
			else
			{
		...
		{
			CustomButtonsMenu.GetStaticButtonPathAndName(currentClass, method, customButton, out _, out var name);
